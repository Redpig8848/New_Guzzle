...
$haomen = array('http://www.soccerbar.cc/forum-14-1.html');
$update_num = 0;
...
//                    $str = str_replace('( 下载次数: 0)', '', $str);
                    if (strpos($txt[0], 'www.soccerbar.cc') !== false) {
                        preg_match("/<img .*?>/", $txt[0], $str);
                        preg_match("/zoomfile=\"(.*?)\"/", $str[0], $src);
                        $str = preg_replace("/src=\".*?\"/", "src=\"http://www.soccerbar.cc/" . $src[1] . "\"", $str[0]);
                    } else {
                        $str = $txt[0];
                    }
//                    print($str);
